...
namespace MAAT {
	...
	void EditorLayer::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<KeyPressedEvent>(MAAT_BIND_EVENT_FN(EditorLayer::OnKeyPressed));
		dispatcher.Dispatch<MouseButtonPressedEvent>(MAAT_BIND_EVENT_FN(EditorLayer::OnMouseButtonPressed));
	}
		...
		if (m_ShowPhysicsColliders)
		{
			// Box Colliders
			{
				...
				for (auto entity : view)
				{
					...
					glm::mat transform = glm::translate(glm::mat4(1.0f), translation)
	...
	{
		std::string filepath = FileDialogs::OpenFile("MAAT Scene (*.maat)\0*.maat\0");
		if (!filepath.empty())
			OpenScene(filepath);
	}
	...
	void EditorLayer::OpenScene(const std::filesystem::path& path)
	{
		...
		if (path.extension().string() != ".maat")
		{
			MAAT_WARN("Could not load {0} - not a scene file", path.filename().string());
			return;
		}
	...
	{
		std::string filepath = FileDialogs::SaveFile("MAAT Scene (*.maat)\0*.maat\0");
		if (!filepath.empty())
		{
	...
	{
		MAAT_CORE_ASSERT(m_SceneState == SceneState::Play || m_SceneState == SceneState::Simulate);
